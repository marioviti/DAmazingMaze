...
solve_task_A_star(Goal,Cost):-
	...
	DpthInit is 0,
	Finit is H,
	solve_task_A_star(Goal,c(Finit,Ginit,P,[]),[],DpthInit,RR,TCost,NewPos),!,
	reverse(RR,[_Init|R]),
	(
		query_world( agent_do_moves, [Agent,R] )->Cost is TCost;
		otherwise->solve_task_A_star(Goal,NCost),Cost is TCost + NCost
	).


acheived(Goal,c(F,G,p(X,Y),Path_to_goal),Agenda,Dpth,[p(X,Y)|Path_to_goal],G,NewPos):-
	Goal=find(Goal_pos),map_adjacent(p(X,Y),_,Goal_pos),\+found(Goal_pos),assert(found(Goal_pos)).

...
solve_task_A_star(go(Goal),Cost):-
	...
	solve_task_A_star(go(Goal),c(Finit,Ginit,P,[]),[],DpthInit,Rpath,Cost,NewPos),	&& c is like a struct with (F,G,P,path_to_P)
...
solve_task_A_star(go(Goal),Current,Agenda,Dpth,RR,Cost,NewPos):-
	...
	add_to_Agenda(Goal,P0,Agenda,[]),
	NewAgenda = [NewCurr|Rest],
	Dpth1 is Dpth+1,
	NewCurr=c(F1,G1,P1,[P0|path_to_P1]),
